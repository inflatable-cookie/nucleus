...
Turn the proven product features into one coherent application. Work from the
app shell inward, removing global or proof-shaped behavior that breaks project
continuity without expanding the visible UI.

## Generation Runway

Current generation goal:

- make project switching restore a complete, predictable working context while
  keeping the normal path visually sparse

Runway bands:

- project-scoped workspace composition and minimal new-project defaults
- shell chrome, project switching, panel recovery, and empty-state cohesion
- shared Goal, Task, selection, and agent-conversation context
- editor, Diff, review, and rework workflow consolidation
...
- `009-longhorn-secondary-system-admission.md` - completed; exact private
  artifact graph, focused adapter modules, and reusable conformance selector
- `010-longhorn-settings-shell.md` - completed; registry, host authority,
  sparse modal, lazy pages, deterministic lifecycle, shared-domain refresh,
  reset, and restart acceptance pass
- `011-provider-and-product-settings.md` - completed; the sparse three-page
  registry, provider defaults, secret-free credential posture, restart, and
  narrow native acceptance pass without invented product settings
- `012-command-catalogue-keymaps-and-palette.md` - completed
- `013-cross-panel-operation-catalogue.md` - completed; Forge host work now
...
- `026-northstar-instruction-and-language-quality-audit.md` - completed; Rust
  1.95 MSRV, both language-quality recorders finalized with honest retained
  findings, and an orientation-first AGENTS surface (card 108)
...
isolated/backing native content, native notifications, and offline queues are
not part of this runway. Cards 050 and 051 keep remote transport and
secondary-window transfer from starting without their missing product rules.

Settings cards 030-035 and roadmap 011 are complete. The registry remains
General, Appearance, and Agent & models because no other durable
user-preference schemas exist. Credential posture and lifecycle requests are
typed and secret-free; provider-managed Codex actions return explicit no-effect
receipts. Card 036 now provides the sealed semantic command catalogue, rooted
contexts, coded product availability, fresh admission, and typed execution
port without creating a generic IPC executor. Card 037 adds the immutable
physical-key preset, durable sparse override domain, caller-scoped keymap host,
conflict and reserved-chord policy, digest-bound mutation, restart, and reset.
Cards 038-044 and 048 are complete. Commands, operations, retained attention,
