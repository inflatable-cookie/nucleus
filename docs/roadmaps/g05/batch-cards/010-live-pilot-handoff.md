# 010 Live Pilot Handoff

Status: completed
Owner: Tom
Updated: 2026-07-25
Completed: 2026-07-25
Milestone: `../003-swallowtail-application-proof-readiness.md`
Auto-start next card: no

## Objective

Freeze one exact Nucleus native pilot envelope and return it for authenticated
call approval before the first provider effect.

## Entry Gates

- cards 007-009 complete
- exact proof runtime checkpointed at
  `11d17810ed68e2b02173c3ae907b83f8db867b58`
- Swallowtail source candidate fixed
- installed Codex exact target and version observed
- intended `gpt-5.4-mini` route fixed; current presence remains the first live
  catalogue stop gate

## Scope

1. Record exact Nucleus, Swallowtail, Codex, model, compatibility, host, and
   topology values.
2. Allocate a fresh explicit proof data root and disposable read-only fixture
   repository.
3. Bind the 12 planned turn scenarios and 3-rerun ceiling from Swallowtail card
   040.
4. Confirm ambient subscription access and no separately metered API route.
5. Confirm safe evidence, cleanup, provider-thread, duration, and stop bounds.
6. Stop before catalogue discovery or turn execution.

## Acceptance

- [x] every exact version and route value is recorded
- [x] no implicit model, access, path, credential, or cost fallback remains
- [x] the state root and fixture repository are disposable
- [x] the live workload and stop conditions match Swallowtail card 040
- [x] provider calls remain separately approved

## Validation

- read-only configuration and installed-version probes only
- `git diff --check`

## Evidence

- `../../../logs/2026-07-25-swallowtail-native-pilot-handoff.md`
- no authenticated catalogue request, model turn, workspace write, provider
  mutation, push, publication, tag, or release occurred
- the handoff returns Swallowtail card 041 for explicit acceptance of the
  ChatGPT-backed 15-turn and 60-minute ceilings

## Stop Conditions

- the selected route is absent or has a different access audience
- exact consumer or Swallowtail source cannot be fixed
- provider spend, state, or cleanup truth is uncertain
- any live provider request would be required to finish the handoff
