...
| Repair authority stays narrow | The branch holds 60 files across 8 commits: 51 audit setup, repair, and instruction files across 4 core audit commits; 8 planning, evidence, and papercut files across 2 closeout commits; and 1 further file across 2 review-wave commits. Of the 51: 21 attributed to 12 Rust repair plans, 4 to 4 TypeScript repair plans, 20 Cargo manifests and 4 policy records from the approved setup tranche, and 2 `AGENTS.md` files (one tool-installed block, one instruction-lane rewrite). Both recorders verify read-only and excluded file hashes at completion and finalization; both finalized without a preservation error. The review wave is recorded outside that finalized evidence. |
...
| Product boundaries survive | No persistence, concurrency, serialization, retry, host/client authority, or public API contract changed. Orchestrator review caught the one place this did not hold: `F-HARN-API-1` derived `Debug` on a type whose unconstrained string fields can carry forbidden terms. It is corrected in the review wave with a redacted manual implementation plus regression tests. Every other type whose `Debug` would render operator or provider content is a retained finding. `effigy qa` passes end to end, including `server:smoke` and the Longhorn consumer boundary check. |
