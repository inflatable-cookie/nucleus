...
use super::types::ForgePullRequestRunnerOutcomePersistenceRecord;

pub(super) const OUTCOME_PREFIX: &str = "forge-pull-request-runner-outcome:";

pub(super) fn write_outcome_record<B>(
    state: &ServerStateService<B>,
    record: &ForgePullRequestRunnerOutcomePersistenceRecord,
...
pub(super) fn decode_outcome_record(
    bytes: &[u8],
) -> LocalStoreResult<ForgePullRequestRunnerOutcomePersistenceRecord> {
    serde_json::from_slice::<ForgePullRequestRunnerOutcomePersistenceRecord>(bytes)
        .map_err(json_error)
}
