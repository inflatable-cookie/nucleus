...
pub use diagnostics::git_push_runner_outcome_diagnostics_from_persisted_records;
pub use types::{
    GitPushRunnerOutcomeDiagnosticsRecord, GitPushRunnerOutcomePersistenceBlocker,
    GitPushRunnerOutcomePersistenceInput, GitPushRunnerOutcomePersistenceRecord,
    GitPushRunnerOutcomePersistenceSet, GitPushRunnerOutcomePersistenceStatus,
...
use types::GitPushRunnerOutcomePersistenceBlocker as Blocker;

pub fn persist_git_push_runner_outcomes<B>(
    state: &ServerStateService<B>,
    input: GitPushRunnerOutcomePersistenceInput,
) -> LocalStoreResult<GitPushRunnerOutcomePersistenceSet>
where
    B: LocalStoreBackend,
{
    let records = input
    ...
    for record in records.iter().filter(|record| {
        record.persistence_status == GitPushRunnerOutcomePersistenceStatus::Persisted
            && !record.duplicate_outcome_detected
    }) {
        write_outcome_record(state, record)?;
    }

    Ok(GitPushRunnerOutcomePersistenceSet {
        outcome_set_id: format!("git-push-runner-outcomes:{}", input.commands.command_set_id),
        records,
...
pub fn read_git_push_runner_outcomes<B>(
    state: &ServerStateService<B>,
) -> LocalStoreResult<Vec<GitPushRunnerOutcomePersistenceRecord>>
where
    B: LocalStoreBackend,
{
...
fn blockers(input: &GitPushRunnerOutcomePersistenceInput) -> Vec<Blocker> {
