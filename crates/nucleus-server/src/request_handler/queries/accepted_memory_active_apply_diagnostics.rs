...
use crate::control_api::{
    AcceptedMemoryActiveApplyDiagnosticsQuery, ServerControlError, ServerQueryResult,
};

pub(crate) fn accepted_memory_active_apply_diagnostics_query<B>(
    handler: &LocalControlRequestHandler<B>,
    query: AcceptedMemoryActiveApplyDiagnosticsQuery,
    ...
    B: LocalStoreBackend + Clone,
{
    if query.project_id.0.trim().is_empty() {
        return Err(ServerControlError::InvalidRequest {
            reason: "accepted memory active apply diagnostics requires a project".to_owned(),
        });
    }
        ...
        .map_err(storage_error)?
    {
        ...
        match decode_persisted_accepted_memory_review_receipt(&record) {
            Ok(receipt) => diagnostic_records
                .push(AcceptedMemoryActiveApplyDiagnosticRecord::PersistedReviewReceipt(receipt)),
    ...
    Ok(ServerQueryResult::AcceptedMemoryActiveApplyDiagnostics(
        AcceptedMemoryActiveApplyDiagnostics::from_records(query.project_id, diagnostic_records),
    ))
}
...
mod tests {
    ...
    #[test]
    fn active_apply_diagnostics_reads_persisted_review_receipts() {
        ...
        assert_eq!(diagnostics.counts.blocked, 0);
    ...
    fn query_diagnostics(
        ...
        let result = accepted_memory_active_apply_diagnostics_query(
            handler,
            AcceptedMemoryActiveApplyDiagnosticsQuery {
                project_id: ProjectId("project:nucleus".to_owned()),
            },
        )
        .expect("accepted memory active apply diagnostics");

        let ServerQueryResult::AcceptedMemoryActiveApplyDiagnostics(diagnostics) = result else {
            panic!("expected accepted memory active apply diagnostics");
        };
