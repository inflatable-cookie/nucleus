...
#[test]
fn git_push_command_descriptors_preserve_remote_target() {
    let record = git_push_command_descriptors(input(true, true, true, Some(remote_target())));

    assert_eq!(record.descriptors.len(), 2);
    assert!(record.descriptors.iter().all(|descriptor| {
        descriptor.status == GitPushCommandDescriptorStatus::Ready
            && descriptor.command_kind == GitPushCommandKind::PushBranch
            && descriptor.remote_target == Some(remote_target())
...
#[test]
fn git_push_command_descriptors_block_failed_admissions() {
    let record = git_push_command_descriptors(input(false, false, false, Some(remote_target())));

    assert_eq!(record.skipped_admission_ids.len(), 2);
    assert!(record.descriptors.iter().all(|descriptor| {
        descriptor.status == GitPushCommandDescriptorStatus::Blocked
            && descriptor
                .blockers
                .contains(&GitPushCommandDescriptorBlocker::AdmissionNotAdmitted)
            && !descriptor.executable_argv_created
            && !descriptor.shell_handoff_created
    }));
}
...
) -> GitPushCommandDescriptorsInput {
    GitPushCommandDescriptorsInput {
