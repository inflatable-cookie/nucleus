//! Persistence and control for stopped provider status/check refreshes.
...
pub use diagnostics::{
    forge_status_check_refresh_control_dto_from_diagnostics,
    forge_status_check_refresh_diagnostics_from_persisted_records,
};
pub use types::{
    ForgeStatusCheckRefreshPersistenceBlocker, ForgeStatusCheckRefreshPersistenceControlDto,
    ForgeStatusCheckRefreshPersistenceDiagnostics, ForgeStatusCheckRefreshPersistenceInput,
    ForgeStatusCheckRefreshPersistenceRecord, ForgeStatusCheckRefreshPersistenceSet,
...
pub fn persist_forge_status_check_refreshes<B>(
    state: &ServerStateService<B>,
    input: ForgeStatusCheckRefreshPersistenceInput,
) -> LocalStoreResult<ForgeStatusCheckRefreshPersistenceSet>
where
    B: LocalStoreBackend,
{
    let records = input
    ...
    for record in records.iter().filter(|record| {
        record.persistence_status == ForgeStatusCheckRefreshPersistenceStatus::Persisted
            && !record.duplicate_refresh_detected
    }) {
        write_refresh_record(state, record)?;
    }

    Ok(ForgeStatusCheckRefreshPersistenceSet {
        persistence_set_id: format!(
            "forge-status-check-refresh-persistence:{}",
...
pub fn read_forge_status_check_refreshes<B>(
    state: &ServerStateService<B>,
) -> LocalStoreResult<Vec<ForgeStatusCheckRefreshPersistenceRecord>>
where
    B: LocalStoreBackend,
{
