...
#[test]
fn forge_pull_request_execution_diagnostics_count_ready_records() {
    let diagnostics = forge_pull_request_execution_diagnostics(input(true, true, true));
...
#[test]
fn forge_pull_request_execution_diagnostics_count_blockers() {
    let diagnostics = forge_pull_request_execution_diagnostics(input(false, false, false));

    assert_eq!(diagnostics.admission_admitted_count, 0);
    assert_eq!(diagnostics.preflight_ready_count, 0);
    assert!(diagnostics.blocker_count >= 6);
}

#[test]
fn forge_pull_request_execution_diagnostics_grant_no_authority() {
    let diagnostics = forge_pull_request_execution_diagnostics(input(true, true, true));
...
fn input(
...
) -> ForgePullRequestExecutionDiagnosticsInput {
    let admissions = crate::forge_pull_request_execution_admission(
    ...
    let preflights = crate::forge_pull_request_execution_preflight(
        crate::ForgePullRequestExecutionPreflightInput {
