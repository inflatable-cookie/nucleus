...
use super::types::GitPushRunnerOutcomePersistenceRecord;

pub(super) const OUTCOME_PREFIX: &str = "git-push-runner-outcome:";

pub(super) fn write_outcome_record<B>(
    state: &ServerStateService<B>,
    record: &GitPushRunnerOutcomePersistenceRecord,
...
pub(super) fn decode_outcome_record(
    bytes: &[u8],
) -> LocalStoreResult<GitPushRunnerOutcomePersistenceRecord> {
    serde_json::from_slice::<GitPushRunnerOutcomePersistenceRecord>(bytes).map_err(json_error)
}
