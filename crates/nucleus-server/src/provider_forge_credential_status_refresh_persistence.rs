...
pub use diagnostics::{
    forge_credential_status_refresh_control_dto_from_diagnostics,
    forge_credential_status_refresh_diagnostics_from_persisted_records,
};
pub use types::{
    ForgeCredentialStatusRefreshPersistenceBlocker,
    ...
    ForgeCredentialStatusRefreshPersistenceSet, ForgeCredentialStatusRefreshPersistenceStatus,
};
...
use types::ForgeCredentialStatusRefreshPersistenceBlocker as Blocker;

pub fn persist_forge_credential_status_refreshes<B>(
    state: &ServerStateService<B>,
    input: ForgeCredentialStatusRefreshPersistenceInput,
) -> LocalStoreResult<ForgeCredentialStatusRefreshPersistenceSet>
where
    B: LocalStoreBackend,
{
    let records = input
    ...
    for record in records.iter().filter(|record| {
        record.persistence_status == ForgeCredentialStatusRefreshPersistenceStatus::Persisted
            && !record.duplicate_refresh_detected
    }) {
        write_refresh_record(state, record)?;
    }

    Ok(ForgeCredentialStatusRefreshPersistenceSet {
        persistence_set_id: format!(
            "forge-credential-status-refresh-persistence:{}",
...
pub fn read_forge_credential_status_refreshes<B>(
    state: &ServerStateService<B>,
) -> LocalStoreResult<Vec<ForgeCredentialStatusRefreshPersistenceRecord>>
where
    B: LocalStoreBackend,
{
...
fn blockers(input: &ForgeCredentialStatusRefreshPersistenceInput) -> Vec<Blocker> {
