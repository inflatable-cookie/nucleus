...
#[test]
fn forge_pull_request_diagnostics_count_ready_records() {
...
#[test]
fn forge_pull_request_diagnostics_count_blockers() {
    let diagnostics = forge_pull_request_diagnostics(input(false, false, false, false));

    assert_eq!(diagnostics.descriptor_ready_count, 0);
    assert_eq!(diagnostics.evidence_reviewable_count, 0);
    assert!(diagnostics.blocker_count >= 14);
}

#[test]
fn forge_pull_request_diagnostics_grant_no_authority() {
    let diagnostics = forge_pull_request_diagnostics(input(true, true, true, true));
...
fn input(
...
) -> ForgePullRequestDiagnosticsInput {
    let descriptors = crate::forge_pull_request_descriptor_records(descriptor_input(
        operator_confirmed,
        remote_ready,
        credential_ready,
    ...
    let evidence =
        crate::forge_pull_request_dry_run_evidence(crate::ForgePullRequestDryRunEvidenceInput {
