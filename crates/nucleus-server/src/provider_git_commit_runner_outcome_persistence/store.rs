...
use super::types::GitCommitRunnerOutcomePersistenceRecord;

pub(super) const OUTCOME_PREFIX: &str = "git-commit-runner-outcome:";

pub(super) fn write_outcome_record<B>(
    state: &ServerStateService<B>,
    record: &GitCommitRunnerOutcomePersistenceRecord,
...
pub(super) fn decode_outcome_record(
    bytes: &[u8],
) -> LocalStoreResult<GitCommitRunnerOutcomePersistenceRecord> {
    serde_json::from_slice::<GitCommitRunnerOutcomePersistenceRecord>(bytes).map_err(json_error)
}
