//! Stopped provider repository metadata refresh records and control DTOs.

mod record_builder;
mod types;

pub use types::{
    ForgeRepositoryMetadataRefreshBlocker, ForgeRepositoryMetadataRefreshControlDto,
    ForgeRepositoryMetadataRefreshInput, ForgeRepositoryMetadataRefreshRecord,
...
pub fn forge_repository_metadata_refresh(
...
) -> ForgeRepositoryMetadataRefreshSet {
    ...
    ForgeRepositoryMetadataRefreshSet {
        refresh_set_id: "forge-repository-metadata-refresh".to_owned(),
        skipped_provider_context_refs: records
            .iter()
            .filter(|record| {
                record.status != ForgeRepositoryMetadataRefreshStatus::ReadyForStoppedRefresh
            })
...
pub fn forge_repository_metadata_refresh_control_dto(
    set: &ForgeRepositoryMetadataRefreshSet,
) -> ForgeRepositoryMetadataRefreshControlDto {
    ForgeRepositoryMetadataRefreshControlDto {
        dto_id: "forge-repository-metadata-refresh-control-dto".to_owned(),
        ...
        blocked_count: status_count(set, ForgeRepositoryMetadataRefreshStatus::Blocked),
