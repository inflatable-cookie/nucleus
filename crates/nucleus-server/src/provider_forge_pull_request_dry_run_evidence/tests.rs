use super::*;
use crate::{
    ForgePullRequestDescriptorInput, GitBranchWorktreeMode, GitBranchWorktreeOutcomeStatus,
    GitCommitMessageSource, GitPushRemoteTarget,
};

#[test]
fn forge_pull_request_dry_run_evidence_composes_reviewable_counts() {
    let record = forge_pull_request_dry_run_evidence(input(true, true, true, true));

    assert_eq!(record.evidence.len(), 2);
    assert!(record.evidence.iter().all(|evidence| {
        evidence.status == ForgePullRequestDryRunEvidenceStatus::Reviewable
            && evidence.changed_path_count == 3
            && evidence.review_comment_count == 0
            && !evidence.pull_request_creation_authority_granted
            && !evidence.pull_request_created
            && !evidence.forge_effect_executed
            && !evidence.provider_effect_executed
            && !evidence.raw_output_retained
    }));
}

#[test]
fn forge_pull_request_dry_run_evidence_blocks_non_ready_descriptors() {
    let record = forge_pull_request_dry_run_evidence(input(false, false, false, true));

    assert_eq!(record.skipped_descriptor_ids.len(), 2);
    assert!(record.evidence.iter().all(|evidence| {
        evidence.status == ForgePullRequestDryRunEvidenceStatus::Blocked
            && evidence
                .blockers
                .contains(&ForgePullRequestDryRunEvidenceBlocker::DescriptorNotReady)
    }));
}
...
) -> ForgePullRequestDryRunEvidenceInput {
    ForgePullRequestDryRunEvidenceInput {
