...
#[test]
fn git_branch_worktree_diagnostics_count_modes_and_records() {
    let diagnostics = git_branch_worktree_diagnostics(input(
...
#[test]
fn git_branch_worktree_diagnostics_grant_no_authority() {
    let diagnostics = git_branch_worktree_diagnostics(input(
        GitBranchWorktreeMode::IsolatedWorktree,
        false,
        false,
...
) -> GitBranchWorktreeDiagnosticsInput {
    let admissions =
        crate::git_branch_worktree_admission_records(crate::GitBranchWorktreeAdmissionInput {
            evidence: evidence(),
            worktree_mode,
        });
    let descriptors = crate::git_branch_worktree_command_descriptors(
        crate::GitBranchWorktreeCommandDescriptorsInput {
            admissions: admissions.clone(),
        },
    );
    let preflights =
        crate::git_branch_worktree_preflight_records(crate::GitBranchWorktreePreflightInput {
