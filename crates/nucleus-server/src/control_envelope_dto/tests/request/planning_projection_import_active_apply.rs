...
#[test]
fn request_envelope_dto_serializes_planning_projection_import_active_apply_diagnostics_query() {
    let request = ServerControlRequest {
        id: ServerControlRequestId(
            "request:dto:planning-projection-import-active-apply".to_owned(),
        ),
        client_id: ClientId("client:desktop".to_owned()),
        kind: ServerControlRequestKind::Query(ServerQuery {
            id: ServerQueryId("query:dto:planning-projection-import-active-apply".to_owned()),
            client_id: ClientId("client:desktop".to_owned()),
    ...
    assert!(matches!(
        ...
        ServerControlRequestKind::Query(ServerQuery {
            kind: ServerQueryKind::PlanningProjectionImportActiveApplyDiagnostics(query),
            ..
