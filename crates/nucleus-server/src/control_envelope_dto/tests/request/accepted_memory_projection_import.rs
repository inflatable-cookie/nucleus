...
#[test]
fn request_envelope_dto_serializes_accepted_memory_projection_import_diagnostics_query() {
    let request = ServerControlRequest {
        id: ServerControlRequestId("request:dto:accepted-memory-projection-import".to_owned()),
        client_id: ClientId("client:desktop".to_owned()),
        kind: ServerControlRequestKind::Query(ServerQuery {
            id: ServerQueryId("query:dto:accepted-memory-projection-import".to_owned()),
            client_id: ClientId("client:desktop".to_owned()),
            kind: ServerQueryKind::AcceptedMemoryProjectionImportDiagnostics(
    ...
    assert!(matches!(
        ...
        ServerControlRequestKind::Query(ServerQuery {
            kind: ServerQueryKind::AcceptedMemoryProjectionImportDiagnostics(query),
            ..
        }) if query.project_id.0 == "project:dto"
    ));
    assert!(json.contains("\"kind\":\"accepted_memory_projection_import_diagnostics\""));
