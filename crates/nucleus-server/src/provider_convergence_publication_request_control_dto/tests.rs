...
#[test]
fn convergence_publication_request_control_dto_serializes_sanitized_counts() {
    let dto = convergence_publication_request_control_dto(persistence("convergence", false));
    let json = serde_json::to_string(&dto).expect("serialize dto");
    let decoded: ConvergencePublicationRequestControlDto =
        serde_json::from_str(&json).expect("deserialize dto");

    assert_eq!(decoded, dto);
    assert_eq!(decoded.persisted_count, 1);
    assert_eq!(decoded.stopped_request_count, 1);
    assert_eq!(decoded.duplicate_count, 0);
    assert!(!decoded.provider_handoff_permitted);
    assert!(!decoded.publish_permitted);
...
#[test]
fn convergence_publication_request_control_dto_reports_blocked_counts() {
    let dto = convergence_publication_request_control_dto(persistence("git", false));
...
) -> crate::ConvergencePublicationRequestPersistenceSet {
    crate::convergence_publication_request_persistence(
