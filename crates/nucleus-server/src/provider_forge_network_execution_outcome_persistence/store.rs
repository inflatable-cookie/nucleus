...
use super::types::ForgeNetworkExecutionOutcomePersistenceRecord;

pub(super) const OUTCOME_PREFIX: &str = "forge-network-execution-outcome:";

pub(super) fn write_outcome_record<B>(
    state: &ServerStateService<B>,
    record: &ForgeNetworkExecutionOutcomePersistenceRecord,
...
pub(super) fn decode_outcome_record(
    bytes: &[u8],
) -> LocalStoreResult<ForgeNetworkExecutionOutcomePersistenceRecord> {
    serde_json::from_slice::<ForgeNetworkExecutionOutcomePersistenceRecord>(bytes)
        .map_err(json_error)
}
