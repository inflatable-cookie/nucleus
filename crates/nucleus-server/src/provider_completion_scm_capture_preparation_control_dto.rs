//! Control DTOs for completion SCM capture-preparation diagnostics.
...
pub fn completion_scm_capture_preparation_control_dto(
    diagnostics: CompletionScmCapturePreparationDiagnosticsRecord,
) -> CompletionScmCapturePreparationControlDto {
    CompletionScmCapturePreparationControlDto {
        dto_id: "completion-scm-capture-preparation-control-dto".to_owned(),
        diagnostics_id: diagnostics.diagnostics_id,
        candidate_count: diagnostics.candidate_count,
        skipped_admission_count: diagnostics.skipped_admission_count,
...
mod tests {
    ...
    #[test]
    fn completion_scm_capture_preparation_control_dto_serializes_sanitized_counts() {
        let dto = completion_scm_capture_preparation_control_dto(diagnostics());
        let json = serde_json::to_string(&dto).expect("serialize dto");
        let decoded: CompletionScmCapturePreparationControlDto =
    ...
    fn diagnostics() -> CompletionScmCapturePreparationDiagnosticsRecord {
