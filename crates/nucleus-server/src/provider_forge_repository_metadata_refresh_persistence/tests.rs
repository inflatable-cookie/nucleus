...
use super::*;
use crate::{ForgeRepositoryMetadataRefreshStatus, ServerStateService};
use nucleus_local_store::SqliteBackend;
use support::*;

#[test]
fn repository_metadata_refresh_persistence_round_trips_sanitized_record() {
    let temp_dir = tempfile::tempdir().expect("temp dir");
    let db = temp_dir.path().join("nucleus.sqlite");
    let state = ServerStateService::new(SqliteBackend::new(db.clone()));

    let set = persist_forge_repository_metadata_refreshes(
        &state,
    ...
    let records = read_forge_repository_metadata_refreshes(&reopened).expect("read");
...
#[test]
fn repository_metadata_refresh_persistence_represents_duplicate_noop() {
    let temp_dir = tempfile::tempdir().expect("temp dir");
    ...
    let first = persist_forge_repository_metadata_refreshes(&state, input(refresh_set.clone()))
        .expect("persist first");
    let duplicate = persist_forge_repository_metadata_refreshes(
        &state,
...
#[test]
fn repository_metadata_refresh_persistence_blocks_missing_evidence_refs() {
    let temp_dir = tempfile::tempdir().expect("temp dir");
    let state = ServerStateService::new(SqliteBackend::new(temp_dir.path().join("db.sqlite")));
    let mut input = input(refresh_set(vec!["provider-context:github:repo".to_owned()]));
    input.evidence_refs.clear();

    let set = persist_forge_repository_metadata_refreshes(&state, input).expect("blocked");
    ...
    assert!(set.records[0]
        ...
        .contains(&ForgeRepositoryMetadataRefreshPersistenceBlocker::MissingEvidenceRef));
}

#[test]
fn repository_metadata_refresh_persistence_blocks_raw_payloads_and_provider_work() {
    let temp_dir = tempfile::tempdir().expect("temp dir");
    let state = ServerStateService::new(SqliteBackend::new(temp_dir.path().join("db.sqlite")));
    let mut input = input(refresh_set(vec!["provider-context:github:repo".to_owned()]));
    ...
    assert!(record
        .persistence_blockers
        .contains(&ForgeRepositoryMetadataRefreshPersistenceBlocker::ProviderNetworkCallRequested));
    assert!(record
        .persistence_blockers
...
#[test]
fn repository_metadata_refresh_persistence_diagnostics_summarize_records() {
    let diagnostics = forge_repository_metadata_refresh_diagnostics_from_persisted_records(vec![
        persisted(
            "one",
            ForgeRepositoryMetadataRefreshPersistenceStatus::Persisted,
        ),
    ...
    let dto = forge_repository_metadata_refresh_control_dto_from_diagnostics(diagnostics.clone());
