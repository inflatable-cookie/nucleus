use crate::{
    ForgeRepositoryMetadataRefreshPersistenceRecord, ForgeRepositoryMetadataRefreshStatus,
};
...
pub fn forge_repository_metadata_refresh_diagnostics_from_persisted_records(
    records: Vec<ForgeRepositoryMetadataRefreshPersistenceRecord>,
) -> ForgeRepositoryMetadataRefreshPersistenceDiagnostics {
    ForgeRepositoryMetadataRefreshPersistenceDiagnostics {
        diagnostics_id: "forge-repository-metadata-refresh-persistence-diagnostics".to_owned(),
        ...
        blocked_refresh_count: refresh_count(
            &records,
...
pub fn forge_repository_metadata_refresh_control_dto_from_diagnostics(
    diagnostics: ForgeRepositoryMetadataRefreshPersistenceDiagnostics,
) -> ForgeRepositoryMetadataRefreshPersistenceControlDto {
    ForgeRepositoryMetadataRefreshPersistenceControlDto {
        dto_id: "forge-repository-metadata-refresh-persistence-control-dto".to_owned(),
...
fn persistence_count(
    records: &[ForgeRepositoryMetadataRefreshPersistenceRecord],
    status: ForgeRepositoryMetadataRefreshPersistenceStatus,
) -> usize {
    records
        .iter()
...
fn refresh_count(
    records: &[ForgeRepositoryMetadataRefreshPersistenceRecord],
    status: ForgeRepositoryMetadataRefreshStatus,
) -> usize {
    records
        .iter()
