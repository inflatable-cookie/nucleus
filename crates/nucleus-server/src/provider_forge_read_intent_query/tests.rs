...
fn read_intent_query_composes_projection_from_local_store_records() {
    ...
    assert!(!result.no_effects.provider_network_call_performed);
    assert!(!result.no_effects.credential_resolution_performed);
}
...
fn read_intent_query_returns_empty_projection_when_store_has_no_sources() {
    ...
    assert!(!result.no_effects.raw_provider_payload_retained);
}
...
fn read_intent_query_control_dto_serializes_sanitized_counts() {
    ...
    assert_eq!(result.control.projection_control.total_count, 4);
    assert!(!result.control.no_effects.provider_effect_executed);
    assert!(!result.control.no_effects.raw_provider_payload_retained);
