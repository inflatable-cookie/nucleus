...
#[test]
fn forge_pull_request_execution_preflight_requires_checks() {
    let record = forge_pull_request_execution_preflight(input(true, true, true));

    assert_eq!(record.preflights.len(), 2);
    assert!(record.preflights.iter().all(|preflight| {
        preflight.status == ForgePullRequestExecutionPreflightStatus::Ready
            && !preflight.pull_request_created
            && !preflight.forge_effect_executed
            && !preflight.provider_effect_executed
            && !preflight.raw_output_retained
    }));
}

#[test]
fn forge_pull_request_execution_preflight_blocks_missing_checks() {
    let record = forge_pull_request_execution_preflight(input(true, false, false));

    assert_eq!(record.skipped_admission_ids.len(), 2);
    assert!(record.preflights.iter().all(|preflight| {
        preflight.status == ForgePullRequestExecutionPreflightStatus::Blocked
            && preflight
                .blockers
...
#[test]
fn forge_pull_request_execution_preflight_blocks_non_admitted_records() {
    let record = forge_pull_request_execution_preflight(input(false, true, true));

    assert!(record.preflights.iter().all(|preflight| {
        preflight
            .blockers
            .contains(&ForgePullRequestExecutionPreflightBlocker::AdmissionNotAdmitted)
    }));
}
...
) -> ForgePullRequestExecutionPreflightInput {
    ForgePullRequestExecutionPreflightInput {
        admissions: crate::forge_pull_request_execution_admission(
            crate::ForgePullRequestExecutionAdmissionInput {
                evidence: crate::forge_pull_request_dry_run_evidence(
                    crate::ForgePullRequestDryRunEvidenceInput {
                        descriptors: crate::forge_pull_request_descriptor_records(
                            descriptor_input(),
                        ),
        ...
        forge_credential_ready,
        remote_branch_visible,
    }
}

fn descriptor_input() -> ForgePullRequestDescriptorInput {
